angular.module('myApp.controllers').controller("trackingController", ['$scope', '$http', 'shellService', 'mapService', function($scope, $http, shellService, mapService){
   ...
   function Init() {
	...
	//This needs a lot of rework - It chokes HARD on too much data due to too many polgyline parts etc.
	//I howevever, needed this so I could implement clicking on a part of the path to get the speed, direction and time at that point easily.
	//Its first crack :-P
		...
		$http({method: 'POST', url: '/system/historicalroute', headers: {'Content-Type': 'application/x-www-form-urlencoded'},
			...
			success(function (result, status, headers, config) {
				if (result.success) {
					...
					//This needs to be rewritten to build up a path object then just splat it over the otherside... it won't ever "replay" like originally intended
					//Right now there are too many calls into other layers (AddtoRoute and its slow.. was fine for a few thousand but after that ....
					//It should instead build up the path and send the whole path over - Add to Route should accept a whole route
					while(true) {
						for(var i = 0; i < vl; i++) {
